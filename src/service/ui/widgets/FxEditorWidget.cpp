...
uint16_t FxEditorWidget::resolveWriteParam_(const LayoutModel& layout,
                                            uint16_t selectedParam,
                                            std::size_t paramCount) noexcept {
    if (paramCount == 0U) {
        return 0U;
    }
    const uint16_t selected = clampParamIndex_(selectedParam, paramCount);

    auto maybeTarget = [selected, paramCount](int32_t readParamIndex,
                                              int32_t targetParamIndex) -> std::optional<uint16_t> {
        if (readParamIndex < 0 || static_cast<uint16_t>(readParamIndex) != selected) {
            return std::nullopt;
        }
        if (targetParamIndex < 0) {
            return selected;
        }
        return clampParamIndex_(static_cast<uint16_t>(targetParamIndex), paramCount);
    };

    for (const LayoutKnob& k : layout.knobs) {
        if (auto idx = maybeTarget(k.paramIndex, k.targetParamIndex); idx.has_value()) {
            return *idx;
        }
    }
    for (const LayoutSwitch& sw : layout.switches) {
        if (auto idx = maybeTarget(sw.paramIndex, sw.targetParamIndex); idx.has_value()) {
            return *idx;
        }
    }
    return selected;
...
FxEditorWidget::LayoutModel FxEditorWidget::buildLayoutModel_(const UiLayoutTemplate& tpl) {
    ...
    tpl.forEachNode([&](const UiLayoutNode& node) {
        ...
        if (node.type == UiLayoutNodeType::Knob) {
            ...
            k.targetParamIndex = target.paramIndex;
            k.bindCanonical = resolved.canonical;
            k.targetCanonical = target.canonical;
        ...
        if (node.type == UiLayoutNodeType::Switch) {
            ...
            sw.targetParamIndex = target.paramIndex;
            sw.bindCanonical = resolved.canonical;
            sw.targetCanonical = target.canonical;
...
WidgetOutput FxEditorWidget::onGesture(UiGesture action, const UiState& rtState, UiNavState& navState) {
    ...
    if ((action == UiGesture::BpmUp || action == UiGesture::BpmDown) &&
        ...
        const float next = std::clamp(cache.values[writeParam] + dir * paramStep_, def.minValue, def.maxValue);
        cache.values[writeParam] = next;
...
UiActionCatalog FxEditorWidget::queryAvailableActions(const UiState& rtState, const UiNavState& navState) const {
...

    {
        ...
        a.def.maxValue = 1.0f;
        a.def.step = 1.0f;
        a.state.enabled = selectedFxAvailable && (descriptor != nullptr);
...
WidgetOutput FxEditorWidget::onAction(UiAction& action, const UiState& rtState, UiNavState& navState) {
    ...
    switch (action.def.id) {
        ...
        case UiAction::Id::SceneFxParamValue: {
            ...
            SlotCache& cache = cacheFor_(track, fxSlot, *descriptor);
            const float next = std::clamp(cache.values[writeParam] + dir * step, def.minValue, def.maxValue);
            cache.values[writeParam] = next;
