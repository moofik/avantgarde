...
TEST_CASE("Engine + QuantizedScheduler: beat-quantized command reaches track on expected block") {
    MockQueue inQ;
    MockQueue outQ;
    MockParamBridge pb;
    auto engine = MakeAudioEngine(&outQ, &pb);

    auto tr = std::make_unique<ProbeTrack>();
    ProbeTrack* trPtr = tr.get();

    int blockIndex = 0;
    trPtr->currentBlock = &blockIndex;
    ...
    QuantizedSchedulerRtExtension scheduler(&inQ, &outQ, &transport, 48000.0);

    engine->setTransportBridge(&transport);
    engine->addRtExtension(&scheduler);

    inQ.push(makeCmd(CmdId::QuantizeMode, -1, 1.0f));
    inQ.push(makePlay());

    const auto ctx = makeCtx(256);

    for (blockIndex = 0; blockIndex < 200; ++blockIndex) {
